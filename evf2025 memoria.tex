...
\end{eqnarray}
%%
donde $\omega_{mn}$ es la diferencia de frecuencias de los niveles y 
$E_R$ es la energía de Rydberg. Este salto es discontinuo si lo vemos 
desde el punto de vista del átomo, porque dijimos que las órbitas son 
estables. Sin embargo, también sabemos que la luz tiene aspecto 
...
arriba se tiene que \'esta es intermitente, con per\'iodos brillantes, en los 
que se emiten muchos fotones, y per\'iodos oscuros, sin emisiones de 
fotones. Muchos experimentos han demostrado el efecto de intermitencia, 
obteniendo la distribuci\'on de la duraci\'on de los per\'iodos brillantes y 
oscuros y otras propiedades 
...
fluorescencia en la transici\'on fuertemente iluminada por un l\'aser  \cite{HePl95,GaKK95}. Este pico fue medido por B\"uhner y Tamm con 
...
obtuvieron una soluci\'on anal\'itica aproximada pero muy precisa para los 
...
estoc\'asticas de Schr\"odinger (EES) \cite{Carm93}. Primero, usando 
EES discont\'inuas, simulamos la formaci\'on de los per\'iodos brillantes 
y oscuros y obtenemos distribuciones de probabilidad de ocurrencia de 
dichos eventos. Destacamos entre ellas, la de los intervalos entre dos 
ciclos de un per\'iodo oscuro seguido de uno brillante, que llamamos 
distribuci\'on de tiempos de retorno, que realza la din\'amica a tiempos 
largos. Despu\'es, con EES difusivas, obtenemos el espectro mediante 
la simulaci\'on del proceso de detecci\'on heterodina de la luz. Finalmente, 
de \cite{CaRG16}, obtenemos resultados complementarios usando el 
m\'etodo de ecuaci\'on maestra para el operador de densidad del \'atomo. 
...
los experimentos que estudiamos solo observamos la luz emitida en 
...
99.95\% al estado $|g\rangle$ y 0.05\% al estado $|a\rangle$. Por 
conveniencia en los c\'alculos num\'ericos, utilizaremos los valores 
$\gamma_e \simeq \gamma =1$ y $\gamma_d =0.05$. La raz\'on 
$\gamma_a$ es en realidad un par\'ametro ajustable para acelerar un 
poco el decaimiento al estado  $|g\rangle$ en el que se usa un l\'aser 
ruidoso para acoplar $|a\rangle$ con un cuarto estado $|c\rangle$ que 
s\'i decae r\'apidamente a $|g\rangle$, pero a\'un cumpliendo la 
condici\'on $\gamma_a < \gamma_d \ll \gamma$ \cite{BuTa00}. Nos 
limitamos al valor $\gamma_a = 0.015 \gamma$.
...
\end{equation}
%%
debe ser normalizada cada intervalo $\delta t$ (del orden de 
$1/100\gamma$), 
%
\begin{figure}[t]
\includegraphics[width=15cm,height=6cm]{fig2_tray_saltos.png}
\caption{\label{fig:3LA-tray} Probabilidad de ocupaci\'on del estado 
excitado $|c_e(t)|^2$; $\Omega = 3.5 \gamma$, 
$\gamma_d = 0.05\gamma$, $\gamma_a = 0.015 \gamma$. 
(Izq.) Segmento de una trayectoria estoc\'astica con per\'iodos brillantes 
y oscuros; (der.) segmento que muestra detalle de las oscilaciones de 
Rabi interrumpidas por la emisi\'on de un fot\'on (l\'ineas verticales) 
y entrada a un per\'iodo oscuro.}
\end{figure}
%% 
%
\begin{eqnarray}
...
%
\begin{equation}
\ket{\bar{\Psi}_c} \to \mathcal{C}_{eg} \ket{\bar{\Psi}_c} 
	= \sqrt{\gamma} \,c_{eg} \ket{g} \,. 
\end{equation}
%%
Aqu\'i tenemos un operador de salto para cada una de las transiciones: 
%
\begin{equation}
...
de fotones, que veremos a continuaci\'on. Para ello se realiz\'o una 
trayectoria muy larga, $\gamma T = 10^6$, para obtener un n\'umero 
muy grande de eventos y, por tanto, excelente estad\'istica. 
...
de manera que  $\rho_{ee}(\infty) \to (\rho_{ee}^{st})_{A2N}$, obteniendo 
%
...
oscuro $\rho_{aa}(t) =1$ y $\rho_{ee}(t) = 0$, por lo que  
%
...
\end{figure}
%% 
Esta distribuci\'on, tambi\'en llamada funci\'on de retardos, se refiere a los 
intervalos entre la emisi\'on de dos fotones consecutivos de la transici\'on 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% new section
\subsection{Distribuci\'on de tiempos de retorno al estado metaestable}
...
saltos son muy frecuentes pero infinitesimales \cite{Carm93}, que simulan 
...
%
\begin{figure}[h]
\includegraphics[width=9cm,height=7cm]{fig6_HtSpectrum.png}
...
de excelente resoluci\'on de este m\'etodo es el del espectro de 
resonancia fluorescente de un \'atomo de dos niveles excitado muy 
fuera de resonancia, $\sim 0.7$ Hz, donde la dispersi\'on es el\'astica, 
es decir, sin mediar absorci\'on \cite{Walther97}.
...
coherencias. Usando la conservaci\'on de la probabilidad, 
$\rho_{ee} +\rho_{gg} +\rho_{aa} =1$, podemos eliminar la ecuaci\'on 
para $\rho_{aa}$. Entonces solo se tiene que resolver un sistema de 
cuatro ecuaciones con cuatro inc\'ognitas que escribimos de manera 
compacta \cite{CaRG16} como 
%
...
brillantes, con evoluci\'on coherente amortiguada con valores propios  
...
%
\begin{figure}[h]
\includegraphics[width=9cm,height=9cm]{fig8_IncSpectra.pdf}
\caption{\label{fig:3LA-WK} Espectro estacionario. 
(a) $\Omega=\gamma_+/4=0.2625 \gamma$, b) $\Omega=3.5 \gamma$ 
(comparar con el de la Fig.\ref{fig:espectroHeterodino}). 
$\gamma_d=0.05$, $\gamma_a=0.015$,  $\Delta=0$. La l\'inea 
punteada en verde corresponde al espectro del \'atomo de dos niveles.}
\end{figure}
%% 
...
$I_{coh}^{A2N} < I_{coh}^{A3N}$, donde $I_{coh}^{A2N}$ se obtiene 
haciendo $q=0$ y reemplazando $\gamma_+$ por $\gamma$. La 
diferencia se refleja en la intensidad del pico angosto:
%
\begin{eqnarray} 	
...
\end{eqnarray}
%%
 El pico es m\'aximo cuando  $\Omega \approx 3\gamma/4$, y peque\~no 
 para campos d\'ebiles $I_{ep} \propto \Omega^2$) y muy fuertes 
 ($I_{ep} \propto \Omega^{-2}$). 
...
En este trabajo presentamos dos contribuciones originales usando 
ecuaciones estoc\'asticas de Schr\"odinger: la distribuci\'on de tiempos 
de retorno al estado metaestable, que enfatiza la din\'amica a tiempos 
largos, usando trayectorias de saltos; y la simulaci\'on del espectro por 
medio de trayectorias cu\'anticas difusivas, mejor adaptadas a la 
realizaci\'on experimental, que las de saltos, y que permiten resolver 
picos espectrales muy angostos, no accesibles por medio de filtros de 
tipo Fabry-Perot.   
